...
A [**resource**](/surveilr/core/concepts/resource/) can be any entity that contains data or is used to facilitate
...
database schema in general and design of the [`uniform_resource`](/surveilr/standard-library/rssd-schema/uniform-resource) table used in
particular. The [`uniform_resource`](/surveilr/standard-library/rssd-schema/uniform-resource) table is built to serve complex requirements
...
In an [RSSD](/surveilr/core/concepts/resource-surveillance/), [**`uniform_resource`**](/surveilr/standard-library/rssd-schema/uniform-resource) represents a central element designed for
...
Let's look at the schema for the [`uniform_resource`](/surveilr/standard-library/rssd-schema/uniform-resource) table to get a better
understanding:
...
The [`uniform_resource`](/surveilr/standard-library/rssd-schema/uniform-resource) table is opinionated in its approach, ensuring that the
...
The [`uniform_resource`](/surveilr/standard-library/rssd-schema/uniform-resource) table interacts with several other tables, forming a
...
The [**`ur_ingest_session`**](/surveilr/standard-library/rssd-schema/ur-ingest-session) table captures sessions during which resources are
...
The [**`ur_ingest_session_fs_path`**](/surveilr/standard-library/rssd-schema/ur-ingest-session-fs-path) table stores file system paths linked to an
...
The [**`ur_ingest_session_imap_account`**](/surveilr/standard-library/rssd-schema/ur-ingest-session-imap-account) table records details about IMAP
...
The [**`ur_ingest_session_imap_acct_folder`**](/surveilr/standard-library/rssd-schema/ur-ingest-session-imap-account-folder) table captures details about the
...
The [**`ur_ingest_session_plm_acct_project`**](/surveilr/standard-library/rssd-schema/ur-ingest-session-plm-account-project) table stores information about
...
The [**`ur_ingest_session_fs_path_entry`**](/surveilr/standard-library/rssd-schema/ur-ingest-session-fs-path-entry/) table records individual file entries
...
- **Key Columns**:
  ...
  - `uniform_resource_id`: Links the file entry to the [`uniform_resource`](/surveilr/standard-library/rssd-schema/uniform-resource) table.

## Compliance and Security Aspects

The [RSSD](/surveilr/core/concepts/resource-surveillance/) database is designed with a focus on compliance and security. Here's
how [`uniform_resource`](/surveilr/standard-library/rssd-schema/uniform-resource) helps achieve this:
...
3. **Detailed Provenance Tracking**: With links to ingestion sessions, file
   paths, and originating devices, the [`uniform_resource`](/surveilr/standard-library/rssd-schema/uniform-resource) table can help
...
The [**`uniform_resource`**](/surveilr/standard-library/rssd-schema/uniform-resource) table is a robust solution for managing resources
with immutability and traceability in mind. By combining the resource with
associated devices, ingestion sessions, and file paths, the [RSSD](/surveilr/core/concepts/resource-surveillance/) database is
built to cater to the high-demand needs of compliance and cybersecurity.

With its opinionated approach, the [`uniform_resource`](/surveilr/standard-library/rssd-schema/uniform-resource) table doesn't just capture
