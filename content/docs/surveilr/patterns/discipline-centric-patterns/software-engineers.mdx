...
Resource surveillance (`surveilr`)  provides the [file ingestion](/surveilr/core/cli/ingest-commands/files#ingest-files) [command](/surveilr/core/cli/cli-commands) for software engineers to execute. This command captures compliance evidence from [Work Product Artifacts (WPAs)](/surveilr/core/concepts/work-product-artifacts/) and store them in a [Resource Surveillance State Database (RSSD)](/surveilr/core/concepts/resource-surveillance/) named `resource-surveillance.sqlite.db`, under the [uniform_resource](/surveilr/standard-library/rssd-schema/uniform-resource/) table. 
...
- To [ingest files](/surveilr/core/cli/ingest-commands/files#ingest-files) in the current directory:
  ```bash
...
The next step is to use `surveilr` to ensure compliance with these policies by [capturing evidence](/surveilr/patterns/discipline-centric-patterns/software-engineers#capturing-compliance-evidence-with-surveilr). After gathering evidence, `surveilr` captures the machine's operating system information and stores it in the [device](/surveilr/standard-library/rssd-schema/device/) table.
...
The next step is to use `surveilr` to ensure compliance with these policies, [capturing](/surveilr/patterns/discipline-centric-patterns/software-engineers#capturing-compliance-evidence-with-surveilr) the necessary details and storing them under the database table.
...
- All developers who use ReactJS as programming language must follow Code [Unit testing](/surveilr/patterns/discipline-centric-patterns/software-engineers#unit-tests) Policy.
- All React developers must ensure they are following React reference Project for React code quality.
