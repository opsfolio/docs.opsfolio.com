...
Resource surveillance (`surveilr`)  provides the [file ingestion](/surveilr/core/cli/ingest-commands/ingest-files) [command](/surveilr/core/cli/cli-commands/) for compliance officers to execute. This command captures compliance evidence from [Work Product Artifacts (WPAs)](/surveilr/core/concepts/work-products-artifacts) and store them in a [Resource Surveillance State Database (RSSD)](/surveilr/core/concepts/resource-surveillance/) named `resource-surveillance.sqlite.db`, under the [uniform_resource](/surveilr/standard-library/rssd-schema/uniform-resource/) table. 
...
- To [ingest files](/surveilr/core/cli/ingest-commands/ingest-files/) in the current directory:
  ```bash
...
The next step is to use `surveilr` to ensure compliance with these policies, [capturing](/surveilr/patterns/discipline-centric-patterns/compliance-officer#capturing-compliance-evidence-with-surveilr) the necessary details and storing them under the database table.
